...
$containerName = "pa-api-v2-project-mysql-$runId"
$networkName = "pa-api-v2-project-mysql-$runId"
...
$testImage = ([string]$env:API_V2_PROJECT_TEST_IMAGE).Trim()
$phpunit = $null
if (-not $testImage) {
    ...
    if (-not (Test-Path $phpunit)) { throw 'PHPUnit is unavailable. Run composer install, set PA_TEST_VENDOR_AUTOLOAD, or set API_V2_PROJECT_TEST_IMAGE to the already-built test image.' }
}
try {
    ...
    if ($testImage) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "API_V2_PROJECT_TEST_IMAGE is unavailable locally: $testImage" }
    ...
    Write-Host "Running API v2 Project synchronization regressions against isolated disposable MySQL 8.4 database $databaseName."
    if ($testImage) {
        docker run --rm --network $networkName --env "API_V2_PROJECT_MYSQL_DSN=$dsn" --env "API_V2_PROJECT_MYSQL_USER=$databaseUser" --env "API_V2_PROJECT_MYSQL_PASSWORD=$databasePassword" --env "API_V2_PROJECT_MYSQL_DATABASE=$databaseName" --env 'API_V2_PROJECT_MYSQL_ALLOW_DESTRUCTIVE=isolated-disposable-only' $testImage php vendor/bin/phpunit tests/Integration/ApiV2ProjectLifecycleMySqlTest.php --do-not-cache-result --colors=never --fail-on-skipped
    } else {
        $env:API_V2_PROJECT_MYSQL_DSN=$dsn;$env:API_V2_PROJECT_MYSQL_USER=$databaseUser;$env:API_V2_PROJECT_MYSQL_PASSWORD=$databasePassword;$env:API_V2_PROJECT_MYSQL_DATABASE=$databaseName;$env:API_V2_PROJECT_MYSQL_ALLOW_DESTRUCTIVE='isolated-disposable-only'
