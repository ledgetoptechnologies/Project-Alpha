...
$testImage = ([string]$env:API_V2_ORG_PROFILE_TEST_IMAGE).Trim()
$phpunit = $null
if (-not $testImage) {
    ...
    if (-not (Test-Path $phpunit)) { throw 'PHPUnit is unavailable. Run composer install, set PA_TEST_VENDOR_AUTOLOAD, or set API_V2_ORG_PROFILE_TEST_IMAGE to the already-built test image.' }
}
...
try {
    ...
    if ($testImage) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "API_V2_ORG_PROFILE_TEST_IMAGE is unavailable locally: $testImage" }
    ...
    if ($testImage) {
        docker run --rm --network $networkName --env "API_V2_ORG_PROFILE_MYSQL_DSN=$dsn" --env "API_V2_ORG_PROFILE_MYSQL_USER=$databaseUser" --env "API_V2_ORG_PROFILE_MYSQL_PASSWORD=$databasePassword" --env "API_V2_ORG_PROFILE_MYSQL_DATABASE=$databaseName" --env 'API_V2_ORG_PROFILE_MYSQL_ALLOW_DESTRUCTIVE=isolated-disposable-only' $testImage php vendor/bin/phpunit tests/Integration/ApiV2DirectoryOrganizationProfileCommandMySqlTest.php tests/Integration/ApiV2DirectoryBindingLifecycleMySqlTest.php tests/Integration/ApiV2DirectoryLifecycleRelationshipMySqlTest.php --do-not-cache-result --colors=never --fail-on-skipped
    } else {
        $env:API_V2_ORG_PROFILE_MYSQL_DSN = $dsn
