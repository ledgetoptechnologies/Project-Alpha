...
$containerName = "pa-api-v2-unit-mysql-$runId"
$networkName = "pa-api-v2-unit-mysql-$runId"
$databaseName = "api_v2_unit_test_$runId"
$databaseUser = 'api_v2_directory_unit'
$rootPassword = [Guid]::NewGuid().ToString('N')
$databasePassword = [Guid]::NewGuid().ToString('N')
$containerStarted = $false
$networkCreated = $false
$repositoryRoot = Split-Path -Parent $PSScriptRoot
$testImage = ([string]$env:API_V2_DIRECTORY_UNIT_TEST_IMAGE).Trim()
$phpunit = $null
if (-not $testImage) {
    ...
    if (-not (Test-Path $phpunit)) { throw 'PHPUnit is unavailable. Run composer install, set PA_TEST_VENDOR_AUTOLOAD, or set API_V2_DIRECTORY_UNIT_TEST_IMAGE to an already-built test image.' }
}
try {
    ...
    if ($testImage) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "API_V2_DIRECTORY_UNIT_TEST_IMAGE is unavailable locally: $testImage" }
    ...
    if ($testImage) {
        docker run --rm --network $networkName --env "API_V2_DIRECTORY_UNIT_MYSQL_DSN=$dsn" --env "API_V2_DIRECTORY_UNIT_MYSQL_USER=$databaseUser" --env "API_V2_DIRECTORY_UNIT_MYSQL_PASSWORD=$databasePassword" --env "API_V2_DIRECTORY_UNIT_MYSQL_DATABASE=$databaseName" --env 'API_V2_DIRECTORY_UNIT_MYSQL_ALLOW_DESTRUCTIVE=isolated-disposable-only' $testImage php vendor/bin/phpunit tests/Integration/ApiV2DirectoryUnitMySqlTest.php --do-not-cache-result --colors=never --fail-on-skipped
    } else {
        $env:API_V2_DIRECTORY_UNIT_MYSQL_DSN=$dsn
        $env:API_V2_DIRECTORY_UNIT_MYSQL_USER=$databaseUser
        $env:API_V2_DIRECTORY_UNIT_MYSQL_PASSWORD=$databasePassword
        $env:API_V2_DIRECTORY_UNIT_MYSQL_DATABASE=$databaseName
        $env:API_V2_DIRECTORY_UNIT_MYSQL_ALLOW_DESTRUCTIVE='isolated-disposable-only'
        & php $phpunit (Join-Path $repositoryRoot 'tests/Integration/ApiV2DirectoryUnitMySqlTest.php') --do-not-cache-result --colors=never --fail-on-skipped
    }
    if ($LASTEXITCODE -ne 0) { throw "Directory unit MySQL tests failed with exit code $LASTEXITCODE." }
} finally {
    Remove-Item Env:API_V2_DIRECTORY_UNIT_MYSQL_DSN -ErrorAction SilentlyContinue
    Remove-Item Env:API_V2_DIRECTORY_UNIT_MYSQL_USER -ErrorAction SilentlyContinue
    Remove-Item Env:API_V2_DIRECTORY_UNIT_MYSQL_PASSWORD -ErrorAction SilentlyContinue
    Remove-Item Env:API_V2_DIRECTORY_UNIT_MYSQL_DATABASE -ErrorAction SilentlyContinue
    Remove-Item Env:API_V2_DIRECTORY_UNIT_MYSQL_ALLOW_DESTRUCTIVE -ErrorAction SilentlyContinue
