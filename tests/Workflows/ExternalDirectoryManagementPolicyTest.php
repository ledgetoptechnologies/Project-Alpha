...
final class ExternalDirectoryManagementPolicyTest extends TestCase
{
    ...
    public function testConfiguredPolicyFailsOpenUntilEveryReplacementRouteExists(): void
    {
        ...
        self::assertFalse($status['effective']);
        self::assertSame('replacement_routes_unavailable', $status['reason']);
    ...
    public function testPreviouslyActivatedOwnershipDoesNotReopenLocalWritersOnHealthDrift(): void
    {
        ...
        self::assertSame('replacement_routes_unavailable',$status['eligibility_reason']);
    ...
    public function testRequiredCapabilitiesRejectLegacyFullAndCoverEveryTopologyOperation(): void
    {
        ...
        foreach (['create','write','archive','restore','delete','bind','read','binding_status.read','organization.assign'] as $fragment) {
    ...
    public function testLiveFlagKeyRevocationScopeSiblingAndAttestationChecksAreIndependent(): void
    {
        ...
        try{
            ...
            $this->pdo->prepare('UPDATE api_keys SET scopes=? WHERE id=1')->execute([implode(',',$required)]);self::assertFalse(api_v2_directory_management_key_ready($this->pdo,7),'Unimplemented scopes are not accepted by the API scope catalog.');
            ...
            $this->pdo->exec("UPDATE api_keys SET revoked_at='2026-01-01' WHERE id=2");self::assertFalse(api_v2_directory_management_key_ready($this->pdo,7));
            ...
            $json=json_encode(['version'=>1,'schemaVersion'=>98,'writerDigest'=>$writerDigest,'backfillDigest'=>str_repeat('a',64)],JSON_THROW_ON_ERROR|JSON_UNESCAPED_SLASHES);$digest=hash('sha256',$json);
    ...
    public function testAuthorizedStatelessApiCommandsRemainOutsideTheBrowserGuard(): void
    {
        ...
        foreach(['directory_create_command_v2.php','directory_client_profile_command_v2.php','directory_organization_profile_command_v2.php','directory_binding_command_v2.php','directory_binding_revision_refresh_v2.php'] as $controller){
