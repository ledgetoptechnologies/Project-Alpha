...
final class ApiV2CatalogInventoryTest extends TestCase
{
    ...
    protected function setUp():void
    {
        ...
        $this->pdo->exec("CREATE TABLE api_keys(id INTEGER PRIMARY KEY,api_v2_application_id INTEGER,revoked_at TEXT);CREATE TABLE api_v2_applications(id INTEGER PRIMARY KEY,application_id TEXT);CREATE TABLE api_v2_history_identity(singleton INTEGER PRIMARY KEY,source_instance_id TEXT,history_epoch TEXT);CREATE TABLE item_library(id INTEGER PRIMARY KEY,portal_public_id TEXT,item_name TEXT,portal_summary TEXT,portal_category TEXT,portal_display_order INTEGER,portal_geometry_requirement TEXT,portal_questions_json TEXT,is_active INTEGER,portal_requestable INTEGER);INSERT INTO api_keys VALUES(7,3,NULL);INSERT INTO api_v2_applications VALUES(3,'{$this->headers['application']}');INSERT INTO api_v2_history_identity VALUES(1,'{$this->headers['source']}','{$this->headers['epoch']}');");
        $insert=$this->pdo->prepare('INSERT INTO item_library VALUES(?,?,?,?,?,?,?,?,?,?)');
        $insert->execute([1,str_repeat('b',32),'Second',null,'Mapping',2,'required','[]',1,1]);
        $insert->execute([2,str_repeat('a',32),'First','Summary','Survey',1,'optional',json_encode([['id'=>'acreage','label'=>'Acres','type'=>'number','required'=>true,'helpText'=>null,'minimum'=>0]]) ,1,1]);
        $insert->execute([3,str_repeat('c',32),'Private',null,'Internal',0,'none','[]',1,0]);
    }
