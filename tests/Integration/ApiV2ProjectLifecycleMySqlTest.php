...
final class ApiV2ProjectLifecycleMySqlTest extends TestCase
{
    ...
    public function testStaleProjectBindingRecoveryIsLeastPrivilegeAndRaceFenced(): void
    {
        ...
        $this->first->exec('UPDATE api_v2_project_authorization_state SET authorization_generation=2 WHERE application_pk=3');$refresh=['commandId'=>'b23e4567-e89b-42d3-a456-426614174000','externalId'=>'mysql-recovery','expectedPublicId'=>str_repeat('a',32),'expectedPriorRevision'=>'1','expectedRevision'=>'2','expectedProjectionSha256'=>$liveHash,'expectedAuthorizationGeneration'=>'1'];self::assertSame(['status'=>409],\api_v2_project_sync_write($this->first,'refresh',$refresh,7,$this->headers,'generation-race'));
        $status=\api_v2_project_binding_status_read($this->first,'mysql-recovery',7,$this->headers,'status-two');$this->first->beginTransaction();$this->first->exec("UPDATE projects SET name='Browser edit two',source_version='v3' WHERE id=1");(new ProjectRevisionService($this->first))->advance(1,'update');$this->first->commit();$refresh['commandId']='c23e4567-e89b-42d3-a456-426614174000';$refresh['expectedAuthorizationGeneration']=$status['payload']['authorizationGeneration'];self::assertSame(['status'=>409],\api_v2_project_sync_write($this->first,'refresh',$refresh,7,$this->headers,'revision-race'));
