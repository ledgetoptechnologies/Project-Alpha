...
function api_v2_directory_organization_profile_command_write(PDO $pdo, string $publicId, array $command, int $apiKeyId, array $headers, string $requestId): array
{
    ...
    try {
        ...
        $receiptStatement = $pdo->prepare('SELECT request_sha256,public_id,CAST(expected_revision AS CHAR) expected_revision,CAST(expected_authorization_generation AS CHAR) expected_authorization_generation,CAST(result_revision AS CHAR) result_revision,result_projection_sha256 FROM api_v2_directory_organization_profile_command_receipts WHERE application_pk=? AND command_id=?' . $lock);
