...
use App\Services\OrganizationProfileMutationService;

require_once __DIR__ . '/organization_schema.php';
require_once __DIR__ . '/address_book.php';
require_once __DIR__ . '/api_v2_directory_revision.php';

function api_v2_directory_organization_profile_address_columns(PDO $pdo): array
{
    if ($pdo->getAttribute(PDO::ATTR_DRIVER_NAME) !== 'sqlite') return pa_existing_organization_address_columns($pdo);
    $columns = array_fill_keys(array_column($pdo->query('PRAGMA table_info(organizations)')->fetchAll(PDO::FETCH_ASSOC), 'name'), true);
    return array_intersect_key($columns, pa_organization_address_definitions());
}

function api_v2_directory_organization_profile_command_parse(string $json): ?array
{
    ...
    $email = strtolower($value['profile']['generalEmail']); $value['profile']['generalEmail'] = $email;
...
function api_v2_directory_organization_profile_positive(string $value): bool
{
    return preg_match('/^[1-9][0-9]{0,18}$/D', $value) === 1
        && !(strlen($value) === 19 && strcmp($value, '9223372036854775807') > 0);
}

function api_v2_directory_organization_profile_result(array $identity, string $publicId, string $revision, string $authorizationGeneration, string $requestId, bool $replayed): array
{
    return ['sourceInstanceId'=>(string)$identity['source_instance_id'], 'applicationId'=>(string)$identity['application_id'], 'historyEpoch'=>(string)$identity['history_epoch'], 'requestId'=>$requestId, 'replayed'=>$replayed,
        'result'=>['resource'=>['type'=>'organization', 'publicId'=>$publicId, 'revision'=>$revision], 'authorizationGeneration'=>$authorizationGeneration]];
}

/** @return array{status:int,payload?:array} */
function api_v2_directory_organization_profile_command_write(PDO $pdo, string $publicId, array $command, int $apiKeyId, array $headers, string $requestId): array
{
    if (preg_match('/^[0-9a-f]{32}$/D', $publicId) !== 1 || $apiKeyId < 1 || $pdo->inTransaction()) throw new InvalidArgumentException('Invalid organization profile command');
    $addressColumns = api_v2_directory_organization_profile_address_columns($pdo);
    ...
    try {
        ...
        $receiptStatement = $pdo->prepare('SELECT request_sha256,public_id,CAST(expected_revision AS CHAR) expected_revision,CAST(expected_authorization_generation AS CHAR) expected_authorization_generation,CAST(result_revision AS CHAR) result_revision,result_projection_sha256 FROM api_v2_directory_organization_profile_command_receipts WHERE application_pk=? AND history_epoch=? AND command_id=?' . $lock);
        $receiptStatement->execute([(int)$identity['application_pk'], $identity['history_epoch'], $command['commandId']]); $receipt = $receiptStatement->fetch(PDO::FETCH_ASSOC);
        ...
        // An exact retry recovers the immutable first result even if a later
        // authorized command advanced either the resource or application.
        // Current key scope was already checked by the controller; consulting
        // mutable state here would turn a lost 200 response into a false 409.
        if ($receipt) {
            $pdo->commit();
            return ['status'=>200, 'payload'=>api_v2_directory_organization_profile_result($identity, $publicId, (string)$receipt['result_revision'], (string)$receipt['expected_authorization_generation'], $requestId, true)];
        }
        ...
        $profileFields = ['name'=>$profile['name'], 'general_email'=>$profile['generalEmail'], 'general_phone'=>$profile['generalPhone'], 'address_line1'=>$profile['addressLine1'], 'address_line2'=>$profile['addressLine2'], 'city'=>$profile['city'], 'state'=>$profile['state'], 'postal_code'=>$profile['postalCode'], 'country'=>$profile['country']];
        $hasProfileChange = false;
        foreach ($profileFields as $field => $value) {
            // Legacy rows may contain either NULL or an empty string. The API
            // exposes both as an empty field, so do not create address history
            // and a new revision solely to normalize that storage detail.
        ...
        $pdo->prepare('INSERT INTO api_v2_directory_organization_profile_command_receipts(application_pk,history_epoch,command_id,request_sha256,public_id,expected_revision,expected_authorization_generation,result_revision,result_projection_sha256) VALUES(?,?,?,?,?,?,?,?,?)')->execute([(int)$identity['application_pk'], $identity['history_epoch'], $command['commandId'], $requestHash, $publicId, $command['expectedRevision'], $command['expectedAuthorizationGeneration'], $result['revision'], $result['projection_sha256']]);
        $pdo->commit(); return ['status'=>200, 'payload'=>api_v2_directory_organization_profile_result($identity, $publicId, (string)$result['revision'], (string)$authorizationGeneration, $requestId, false)];
