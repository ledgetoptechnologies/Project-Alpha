...
// Front controller must dispatch this before the interactive session/router.
...
if ($contentLength !== null && (!ctype_digit((string)$contentLength) || (int)$contentLength > 32 * 1024)) { http_response_code(413); exit; }
$stream = fopen('php://input', 'rb');
if ($stream === false) { http_response_code(400); exit; }
$body = stream_get_contents($stream, 32 * 1024 + 1); fclose($stream);
if (!is_string($body) || strlen($body) > 32 * 1024) { http_response_code(413); exit; }
$command = api_v2_directory_binding_command_parse($body);
if ($command === null) { http_response_code(400); exit; }
try {
...
} catch (Throwable $error) { error_log('[ApiV2DirectoryBinding] database unavailable: ' . get_class($error)); http_response_code(503); exit; }
...
try {
    $outcome = api_v2_directory_binding_command_write($pdo, $type, $command, (int)$key['id'], [
    ...
    if ($outcome['status'] !== 200) { http_response_code($outcome['status']); exit; }
    $json = json_encode($outcome['payload'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR);
    if (strlen($json) > 64 * 1024) throw new RuntimeException('Binding response limit');
    echo $json;
} catch (Throwable $error) { error_log('[ApiV2DirectoryBinding] command unavailable: ' . get_class($error)); http_response_code(503); }
