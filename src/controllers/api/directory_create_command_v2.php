...
$stream = fopen('php://input', 'rb'); if ($stream === false) { http_response_code(400); exit; }
$body = stream_get_contents($stream, 32 * 1024 + 1); fclose($stream);
if (!is_string($body) || strlen($body) > 32 * 1024) { http_response_code(413); exit; }
$command = api_v2_directory_create_command_parse($type, $body); if ($command === null) { http_response_code(400); exit; }
try {
...
try {
    ...
    if (isset($outcome['payload'])) {
        ...
        if (strlen($json) > 64 * 1024) throw new RuntimeException('Directory create response limit');
        echo $json;
    }
} catch (Throwable $error) { error_log('[ApiV2DirectoryCreate] command unavailable: ' . get_class($error)); http_response_code(503); }
