...
$cursor=null;$limit=100;$apply=false;$dry=false;$confirmed=false;$maintenance=false;$attest=false;
...
if($attest){try{$digest=api_v2_project_backfill_attestation_persist(migration_connection());fwrite(STDOUT,'Attestation: '.$digest.".\n");}catch(Throwable$error){fwrite(STDERR,'Project attestation refused: '.$error->getMessage()."\n");exit(1);}}
